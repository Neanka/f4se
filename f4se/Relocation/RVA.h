...
public:
	...
	sig_scan_processor(const char * iniName, version_map versionData, const char * sig = nullptr, int relOffset = 0, int dataOffset = 0, int instructionLen = 0, indirection indirections = indirection()) : m_result(0)
	{
		...
		else if (sig)
		{
			...
			std::string hs;
			hs = sig + std::string("|") + std::to_string(relOffset << 0x10 | (dataOffset & 0xFF) << 0x8 | instructionLen & 0xFF);
			if (indirections.size()>0)
			{
				for (int i = 0; i < indirections.size(); i++)
				{
					hs += std::string("|") + std::to_string(indirections[i][0] << 0x10 | (indirections[i][1] & 0xFF) << 0x8 | indirections[i][2] & 0xFF);
				}
			}
			size_t hash = hash_fn(hs);
		...
		else /*if (iniName && plugin_info.plugin_name)*/
		{
			...
			else if (sig != nullptr)
			{
				...
				if (indirections.size()>0)
				{
					...
					for (int i = 0; i < indirections.size(); i++)
					{
						scan_result += indirections[i][0];
						if (indirections[i][1])
						{
							sig_scan_util::read_memory(scan_result + indirections[i][1], &rel322, sizeof(int32_t));
							scan_result += indirections[i][2] + rel322;
						}
					}
				}
	...
	sig_scan_processor(const char * iniName, const char * sig = nullptr, int relOffset = 0, int dataOffset = 0, int instructionLen = 0, indirection indirections = indirection()) \
		: sig_scan_processor(iniName, { {} }, sig, relOffset, dataOffset, instructionLen, indirections) {  }

	sig_scan_processor(version_map versionData, const char * sig = nullptr, int relOffset = 0, int dataOffset = 0, int instructionLen = 0, indirection indirections = indirection()) \
		: sig_scan_processor(nullptr, versionData, sig, relOffset, dataOffset, instructionLen, indirections) {  }

	sig_scan_processor(const char * sig = nullptr, int relOffset = 0, int dataOffset = 0, int instructionLen = 0, indirection indirections = indirection()) \
		: sig_scan_processor(nullptr, { {} }, sig, relOffset, dataOffset, instructionLen, indirections) {  }

	sig_scan_processor(const char * iniName, relative_address relMem, const char * sig = nullptr, int relOffset = 0, int dataOffset = 0, int instructionLen = 0, indirection indirections = indirection()) \
		: sig_scan_processor(iniName, { { CURRENT_RELEASE_RUNTIME, relMem } }, sig, relOffset, dataOffset, instructionLen, indirections) {  }

	sig_scan_processor(relative_address relMem, const char * sig = nullptr, int relOffset = 0, int dataOffset = 0, int instructionLen = 0, indirection indirections = indirection()) :\
		sig_scan_processor(nullptr, { { CURRENT_RELEASE_RUNTIME, relMem } }, sig, relOffset, dataOffset, instructionLen, indirections) {  }
